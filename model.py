...
def build_training_dataset(fp2_pace_df, actual_race_results_df, historical_team_results_df):
    # ... (Keep your existing merge code here) ...
    ...
    df = pd.merge(fp2_pace_df, team_history, on='Team', how='left')
    ...
    # NEW: Create the track-agnostic Pace Rank!
    df['Pace_Rank'] = df['FP2_Avg_Pace_s'].rank()
    
    return df

def train_rf_model(training_df):
    # NEW: Use Pace_Rank instead of raw seconds!
    features = ['Pace_Rank', 'Team_Hist_Pos']
